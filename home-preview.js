(() => {
  ...
  function refresh() {
    ...
    if (recommendation) {
      ...
      const confirmed=state.profile.confirmed, names={boundary:'경계',direction:'방향',structure:'골조'};
      ...
      const covers={boundary:['I','나를 지키는\n마음의 경계.'],direction:['II','내가 머물\n다음 방향.'],structure:['III','다시 세울\n나의 골조.']};
      $('for-you-state').textContent=confirmed?'내가 확인한 고민':'나를 알아가는 질문';
      $('for-you-address').textContent=state.profile.nickname?`${state.profile.nickname}님에게 고른 한 편.`:confirmed?'당신이 들려준 이야기에서.':'당신에게 먼저 묻고 싶어요.';
      $('home-personal-question').textContent=shortQuestions[confirmed?state.profile.concern:seed.concern];
      $('home-personal-reason').textContent=confirmed?recommendation.reason:'지금의 상황을 먼저 듣고, 다음 기록을 골라드릴게요.';
      ...
      $('for-you-book-label').textContent=ready?`${confirmed?'나를 위해 고른 기록':'먼저 펼쳐볼 기록'} / ${names[recommendation.id]}`:'나의 속도로';
      $('for-you-book-number').textContent=ready?cover[0]:'—';
      $('for-you-book-title').textContent=ready?cover[1]:'서두르지\n않아도 괜찮아요.';
      $('for-you-book-link').textContent=ready?'이 기록 미리 읽기':'천천히 둘러보기';
