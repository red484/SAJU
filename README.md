...
홈은 조각상 표지와 세 가지 미리보기(오늘의 안내·기록·서재)로만 구성합니다. 실제 기능은 다음 페이지에 분리되어 있습니다.
...
기록 읽기 페이지의 하강 지면은 화면 너비를 채우는 긴 세로 그림과 그 아래 여백의 타이틀로 구성됩니다. 그림은 일반 스크롤로 읽으며 고정·낙하 애니메이션이 없습니다. 서재의 네 조각이 합쳐지는 항해 장면만 스크롤 위치에 맞춰 움직이며 스크롤 입력은 가로채지 않습니다. 작은 화면·큰 글씨·동작 줄이기 설정에서는 고정 장면을 풀어 일반 문서로 표시합니다. 각 장면에 건너뛰기 링크가 있습니다. `node validate-scroll.mjs`로 동작 분기와 스크롤 갱신을 검증합니다.
