...
메인 홈에 하강 장면과 네 조각이 합쳐지는 항해 장면이 추가되어 있습니다. 스크롤 위치에 맞춰 움직이며 스크롤 입력은 가로채지 않습니다. 작은 화면·큰 글씨·동작 줄이기 설정에서는 고정 장면을 풀어 일반 문서로 표시합니다. 각 장면에 건너뛰기 링크가 있습니다. `node validate-scroll.mjs`로 동작 분기와 스크롤 갱신을 검증합니다.
