...
홈 첫 화면의 맞춤 달력은 이번 주 남은 날 중 추천 날짜, 월 이동, 날짜별 안내, 작은 실천과 관련 기록을 제공합니다. 관심사(관계·일·돌봄)를 바꾸면 추천 날짜와 기록 순서가 함께 바뀝니다. 기존 온보딩은 변경하지 않았으며, 같은 세션에 남아 있는 회복 방식과 부족한 것에 대한 답변만 참고합니다. 생년월일을 새로 저장하지 않고 관심사 선택만 현재 기기에 보관합니다. 날짜별 안내는 관심사·요일에 따른 편집 예시이며 실제 사주 계산이나 포스텔러 데이터 연동이 아닙니다.
